...
namespace nc
{
	...
	void Renderer::CreateWindow(const std::string& title, int width, int height)
	{
		...
		//set compatibility -> old vs new version
		SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_COMPATIBILITY);

		// tells how many buffers ->
		SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
		//use the video card - hardware
		SDL_GL_SetAttribute(SDL_GL_ACCELERATED_VISUAL, 1);

		//swap after every frame of buffer - > swap vsync
		...
		glEnable(GL_DEBUG_OUTPUT); // added 10/4
		glDebugMessageCallback(DebugCallback, 0); // added 10/4
